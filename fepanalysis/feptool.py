...
def main():
    ...
    if args.estimator =='Zwanzig_Estimator':
        ...
        print("ZW: ",Zwanzig_dG)
        print("TI: ", TI_dG)
        u_nk_df,states_dicts,lambdas_list= estimators.Estimators.Create_df_BAR_MBAR(State_A_df, State_B_df)
        BAR_df= estimators.BAR().fit(u_nk_df)
        print('BAR: ',BAR_df.delta_f_.loc[0.00, 1.00])
        BAR_df2=estimators.Estimators.Create_df_BAR_MBAR_2(states_dicts,State_A_df,lambdas_list,None)
        print('BAR2',BAR_df2)

...
def main():
    ...
    if args.estimator =='Zwanzig_Estimator':
        ...
        DG_df = estimators.Estimator.Zwanzig(dEs,None)
        dU_dH_df=estimators.Estimator.Create_df_TI(State_A_df, State_B_df)
        Zwanzig_dG = DG_df['dG_Average'].iloc[-1]
        TI_dG = estimators.Estimator.TI(dU_dH_df)
        print("ZW: ",Zwanzig_dG )
        print("TI: ", TI_dG)
        
    if args.convergence_analysis is not None:
        print(eval('estimators.Estimator.'+args.convergence_analysis[0]))
        args.convergence_analysis=args.convergence_analysis[0].split(',')
        convergenc_df = estimators.Estimator.Convergence(dEs,eval('estimators.Etimators.'+args.convergence_analysis[0]), int(args.convergence_analysis[1]), int(args.convergence_analysis[2]), int(args.convergence_analysis[3]))
        print(convergenc_df)
